...
Django は **開発用 HTTP サーバー（`runserver`）が動いている間だけ** ブラウザからページを表示できます。エディタや Cursor だけ開いていても、サーバーを起動していないとページは表示されません。

### 1. 初回のみ（または環境を作り直したとき）

プロジェクトのルート（`manage.py` があるディレクトリ）で、上記 **セットアップ** のとおり `venv` の有効化・`pip install`・`migrate` まで済ませてください。

### 2. 開発サーバーを起動

ターミナルで次を実行し、**起動したまま** にします。

```bash
cd house    # プロジェクトルートへ（既にいる場合は不要）
source venv/bin/activate   # Windows: venv\Scripts\activate
